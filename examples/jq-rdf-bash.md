...
Our goal is to figure out which keys have values which are univerally unique. In this example we know it's the `.id` values. While an ec2 instance might share the same name. They would not share the same `.id`. So before we load it into dgraph. Let's indicate that. 
